...
interface Props {
  comisionEmpledo: ComisionEmpledo;
  empleados: Empleado[];
  comisiones: Comision[];
}

const ComisionEmpledoEdit: React.FC = () => {
  const { comisionEmpledo, empleados, comisiones } = usePage<PageProps<Props>>().props;

  const { data, setData, put, processing, errors } = useForm({
    empleado_id: String(comisionEmpledo.empleado_id),
    comision_id: String(comisionEmpledo.comision_id),
  });

  // Keep form data in sync if prop changes
  useEffect(() => {
    setData({
      empleado_id: String(comisionEmpledo.empleado_id),
      comision_id: String(comisionEmpledo.comision_id),
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    put(route("spad.updatecomision_empledo", { comision_empledo: comisionEmpledo.id }));
  };
  ...
  return (
    <AppLayout
      breadcrumbs={[
        ...
        { title: "Editar Asignación", href: "#" }
      ]}
    >
      <div className="max-w-2xl mx-auto p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-amber-900">Editar Asignación de Comisión</h1>
          <p className="text-sm text-amber-700 mt-1">Actualiza la comisión asignada para el empleado.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white border border-amber-100 p-6 rounded-lg shadow-sm">
          <div className="mb-6">
            ...
            {errors.empleado_id && (
              <p className="mt-1 text-sm text-red-655 font-medium">{errors.empleado_id}</p>
            )}
          </div>
          ...
          <div className="mb-6">
            ...
            {errors.comision_id && (
              <p className="mt-1 text-sm text-red-655 font-medium">{errors.comision_id}</p>
            )}
          </div>
          ...
          <div className="flex justify-end gap-3 mt-8">
            <button
              ...
              className="px-4 py-2 border border-gray-300 text-gray-700 bg-amber-500 rounded-md hover:bg-amber-500 transition font-medium text-sm"
            >
              Cancelar
            </button>
            <button
            ...
            >
              {processing ? "Guardando..." : "Actualizar Asignación"}
            </button>
          </div>
        </form>
      </div>
...
export default ComisionEmpledoEdit;
