...
export default function FormaPagoEdit() {
  const { formaPago } = usePage<{ formaPago: FormaPagoData }>().props;

  const { data, setData, put, processing, errors } = useForm({
    nombre_forma_pago: formaPago.nombre_forma_pago || '',
    descripcion: formaPago.descripcion || '',
    nombre_corto: formaPago.nombre_corto || '',
  });

  const submit = (e: FormEvent) => {
    e.preventDefault();
    put(route('spad.updateformapago', { formaPago: formaPago.id }));
  };
      ...
      <div className="mx-auto max-w-2xl p-4">
        <div className="rounded-lg border border-amber-200 bg-white p-6 shadow-sm">
          <div className="mb-4 flex items-center justify-between">
            <h1 className="text-2xl font-bold text-amber-700">Editar forma de pago</h1>
            <a
